...
abstract class AbstractSimulationUIItemTest extends AbstractUIItemTest {
    ...
    public SimulationRun createResults(String name, int index) {
        ...
        new SingleValueResult(simulationRun: run, path: path1, collector: collector1, field: field, period: 0, iteration: 1, value: 1000).save()
        new SingleValueResult(simulationRun: run, path: path2, collector: collector1, field: field, period: 0, iteration: 2, value: 1000).save()
        new SingleValueResult(simulationRun: run, path: path1, collector: collector1, field: field, period: 0, iteration: 1, value: 500).save()
        new SingleValueResult(simulationRun: run, path: path2, collector: collector1, field: field, period: 0, iteration: 2, value: 1).save()
