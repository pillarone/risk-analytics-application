...
//            FileOutputStream fos = new FileOutputStream(testExportFile)
        ...
        reportHelper.metaClass.getPageTitle = {ResultPathParser parser, String path, String type, int period ->
            return path
        }
...
//            map.put(PathType.REINSURANCE, ["path1", "path2", "path3", "path4"] as List<String>)
        ...
        model.metaClass.getChartDataSource = { int period, List<String> path ->
            //pdf chart
...
//            xyPairs.eachWithIndex {List list, int index ->
            ...
            return ReportUtils.getRendererDataSource("pdfChart", JasperChartUtils.generatePDFChart(seriesMap))
            /*
             Map seriesMap = [:]
        componentPaths.each {String path ->
            fieldNames.each {String fieldName ->
                Map dbValues = getValues(periodIndex, path, collector, fieldName)
                if (dbValues) {
                    try {
                        seriesMap[parser.getFiedName(path, fieldName)] = JEstimator.gaussKernelBandwidthPdf(dbValues["values"], calcBandwidth(dbValues), false)
                    } catch (Exception ex) {
                    }
                }
            }
        }
        return getPDFDataSource(JasperChartUtils.generatePDFChart(seriesMap))
             */
...
//            JRBeanCollectionDataSource jrBeanCollectionDataSource = new JRBeanCollectionDataSource(currentValues);
        ...
        model.metaClass.getFieldFunctionValues = {List<String> path, int periodIndex ->
