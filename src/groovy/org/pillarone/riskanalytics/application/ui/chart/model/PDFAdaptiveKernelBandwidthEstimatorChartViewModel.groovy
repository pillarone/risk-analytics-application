 ...
 * Time: 14:55:52
 */
class PDFAdaptiveKernelBandwidthEstimatorChartViewModel extends AdaptiveKernelBandwidthEstimatorChartViewModel implements PDFChartViewModel {
    static double spinnerStepSize = 0.1 //stepsize depending on t

    public PDFAdaptiveKernelBandwidthEstimatorChartViewModel(String title, SimulationRun simulationRun, List<ResultTableTreeNode> nodes) {
    ...
    protected void addToSeries(XYSeries seriesPDF) {
        ...
        List pdfValues = JEstimator.adaptiveKernelBandwidthPdf(observations, priorBandwidth, false)
    ...
    protected void writeInsetContent(ChartInsetWriter writer) {
        writer.writeInset("Prior bandwidth (Gaussian)", priorBandwidth)
    ...
    public ULCSpinnerNumberModel getSpinnerModel() {
        double t0r = Math.round(priorBandwidth0 * 10) / 10
    ...
    public void setBandwith(double newBandwidth) {
        priorBandwidth0 = newBandwidth
        fireModelChanged()
    }
